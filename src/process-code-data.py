...
def construct_prompts(data, system_prompt):
    ...
    for item in data:
        ...
        meta = item.get('meta', {})
        repo_name = meta.get('max_stars_repo_name', '[Not Available]')
        repo_url = f"https://github.com/{repo_name}" if repo_name != '[Not Available]' else '[Not Available]'
        file_path = meta.get('max_stars_repo_path', '[Not Available]')
        file_url = f"{repo_url}/blob/master/{file_path}" if file_path != '[Not Available]' else '[Not Available]'
