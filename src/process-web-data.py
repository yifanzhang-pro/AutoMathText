# Autonomous Data Selection with Language Model (Qwen-72B) on web texts (Common Crawl, OpenWebMath) 
# Author: Yifan Zhang
...
You are ChatGPT, the most capable large language model equipped with extensive expertise in mathematics and coding, particularly skilled in complex reasoning and problem-solving. In the following interaction, I will provide you with a text excerpt from a website. Your task is to evaluate whether this text contains elements of mathematical intelligence and if it is suitable for educational purposes for YOURSELF in the field of mathematics. Please respond with only YES or NO
<\system>
"""
...
def process_batches(original_filename, prompts, llm, sampling_params, original_data):
    for batch_num in range(0, len(prompts), BATCH_SIZE):
        ...
        for i, output in enumerate(outputs):
            ...
            q1_logprob_yes = max(q1_logprob_yes, output.outputs[0].logprobs[0].get(Yes_ID, -100))
            ...
            q1_logprob_no = max(q1_logprob_no, output.outputs[0].logprobs[0].get(No_ID, -100))
